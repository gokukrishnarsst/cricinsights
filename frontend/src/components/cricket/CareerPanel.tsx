...
import type { PlayerCareerReport } from "@/types/cricket";
import type { FormatBatting, FormatBowling } from "@/lib/db/seed/types";
...
function BatRow({ label, b }: { label: string; b: FormatBatting }) {
  return (
    <tr className="border-b border-line/60 last:border-0">
      ...
      <td className="px-3 py-2 text-ink-dim">{b.hundreds}/{b.fifties}</td>
  ...
  return (
    <section className="space-y-5">
      <div>
        ...
        <p className="mt-2 text-[14px] leading-relaxed text-ink-soft">{bioSummary}</p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="panel p-4">
          ...
          <ul className="space-y-2 text-[13px] text-ink-soft">
            {coachStrengths.map((s) => (
              <li key={s.label}>
                <span className="font-semibold text-ink">{s.label}:</span> {s.evidence}
              </li>
            ))}
          </ul>
        </div>
        <div className="panel p-4">
          ...
          <ul className="space-y-2 text-[13px] text-ink-soft">
            {coachGaps.map((g) => (
              <li key={g.label}>
                <span className="font-semibold text-ink">{g.label}:</span> {g.evidence}
              </li>
            ))}
          </ul>
        </div>
