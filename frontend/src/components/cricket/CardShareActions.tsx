...
export default function CardShareActions({
  ...
  return (
    <div className="flex flex-wrap items-center justify-center gap-2">
      {canShare && (
        <button type="button" className={btn} onClick={nativeShare} disabled={busy}>
          <Share2 size={16} />
          Share
        </button>
      )}
      ...
      <button type="button" className={btn} onClick={download} disabled={busy}>
        {done === "download" ? <Check size={16} className="text-accent" /> : <Download size={16} />}
        {done === "download" ? "Saved" : "Download"}
      </button>
      <button type="button" className={btn} onClick={copyImage} disabled={busy}>
        {done === "copy" ? <Check size={16} className="text-accent" /> : <Copy size={16} />}
