...
const FONT_MEDIUM = "var(--font-din-cond), 'Saira Condensed', sans-serif";
const FONT_COND = "var(--font-din-cond), 'Saira Condensed', sans-serif";
const FONT_BOLD = "var(--font-din-cond), 'Saira Condensed', sans-serif";
...
function CricketPlayerCard({ card }: { card: RatedPlayerCard }) {
  const t = resolveCardTheme(card.tier);
  const ink = t.ink;
    ...
    <div className="cric-card-frame" style={wrap}>
      ...
      {card.player.teamLogoUrl ? (
        ...
        card.player.league && (
          ...
          >
            {card.player.league === "IPL" ? "IPL" : "100"}
          </div>
        )
      )}
