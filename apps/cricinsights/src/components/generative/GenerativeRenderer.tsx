...
function StatsTable({
  ...
  return (
    <div className="animate-rise-soft overflow-hidden rounded-xl border border-line/80">
      {title ? (
...
function LeagueComparisonViz({
  ...
  return (
    <div className="panel animate-fade-up space-y-4 p-5 sm:p-6">
    ...
    <div className="grid gap-3 sm:grid-cols-2">
      {items.map((item) => (
        <a
          ...
          href={
            item.prompt
              ? `/chat?q=${encodeURIComponent(item.prompt)}`
              : '/chat'
          }
...
const CHART_TYPES: UIComponentType[] = [
  'radar_chart',
  'bar_comparison',
  'league_comparison',
  'phase_breakdown',
...
export function RenderGenerativeItem({
  item,
  comparisonVariant = 'embedded',
}: {
  item: UIComponent;
  comparisonVariant?: 'arena' | 'embedded';
}) {
  switch (item.type) {
    case 'text':
    ...
    case 'player_card':
      return (
        <div className="animate-bat-swing mx-auto max-w-[280px]">
          <TiltCard className="card-3d-hover">
    ...
    case 'comparison_card':
      if (!item.comparison?.playerA || !item.comparison?.playerB) {
        return <GenerativeSkeleton variant="wide" />;
      }
      return (
        <div className="animate-rise-soft space-y-4">
          <ComparisonView
    ...
    case 'radar_chart':
      return (
        <div className="panel animate-fade-up p-5">
          {item.title ? <div className="eyebrow mb-4">{item.title}</div> : null}
...
function itemEntranceClass(type: UIComponentType): string {
  if (CHART_TYPES.includes(type)) return 'animate-fade-up';
  if (type === 'stats_table') return 'animate-rise-soft';
  return 'animate-rise-soft';
}
  ...
  return (
    <div className="mt-4 space-y-6">
      {items.map((item, i) => (
        <div
          key={`${item.type}-${i}`}
          className={itemEntranceClass(item.type)}
